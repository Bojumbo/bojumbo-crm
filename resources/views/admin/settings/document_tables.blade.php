...
<div x-data="{ 
    ...
    <div class="bg-card border border-notion-border rounded-notion overflow-hidden shadow-sm">
        <div class="p-6 border-b border-notion-border bg-white/5 flex items-center justify-between">
            <div>
                <h3 class="font-bold text-lg text-white">{{ __('Document Tables') }}</h3>
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            @foreach($tables as $table)
                <div class="p-4 rounded-xl border border-notion-border bg-white/[0.02] space-y-4 hover:border-notion-blue/30 transition-colors cursor-pointer group" @click="openEdit({{ json_encode($table) }})">
                    <div class="flex items-center justify-between">
                        <h4 class="font-bold text-white text-base group-hover:text-notion-blue transition-colors">{{ $table->name }}</h4>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            @foreach($table->columns as $col)
                                <div class="bg-white/5 px-2 py-1 rounded text-[10px] text-notion-text-secondary truncate border border-white/5 flex items-center justify-between" title="{{ $col['label'] }}">
                                    <span>{{ $col['label'] }}</span>
    ...
    <div x-show="open" 
        ...
        <div class="bg-card border border-notion-border w-full max-w-4xl rounded-xl shadow-2xl overflow-hidden p-8 space-y-6 max-h-[90vh] overflow-y-auto custom-scrollbar" @click.away="open = false">
            <h3 class="text-xl font-bold text-white tracking-tight" x-text="isEdit ? '{{ __('Edit Table Configuration') }}' : '{{ __('Create Table Configuration') }}'"></h3>
            ...
            <form :action="isEdit ? '/admin/document-tables/' + editId : '{{ route('admin.settings.document_tables.store') }}'" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div class="flex items-center gap-2 pt-6">
                        <input type="checkbox" name="show_total" value="1" x-model="showTotal" id="show_total" class="rounded border-notion-border bg-white/5 text-notion-blue focus:ring-notion-blue">
                        <label for="show_total" class="text-sm text-white select-none">{{ __('Show Total Row?') }}</label>
                    </div>
                </div>

                <!-- Global Styles -->
                <div class="bg-white/5 p-4 rounded-xl border border-notion-border/50">
                        ...
                        <template x-for="(col, index) in cols" :key="index">
                            <div class="bg-white/5 p-4 rounded-xl border border-white/5 space-y-3">
                                <div class="flex gap-3 items-center">
                                    <div class="w-6 h-6 rounded-full bg-white/5 flex items-center justify-center text-[10px] text-notion-text-secondary font-bold" x-text="index + 1"></div>
                                    <div class="flex-1">
                                 ...
                                 <div class="grid grid-cols-5 gap-3 ps-9 items-end">
                                     ...
                                     <div class="flex items-center gap-2 pb-1.5 overflow-hidden">
                                         <div class="flex items-center gap-1.5">
                                             <input type="checkbox" :name="'columns['+index+'][styles][header_bold]'" x-model="col.styles.header_bold" value="1" :id="'h_bold_'+index" class="rounded border-notion-border bg-white/5 text-notion-blue">
                                             <label :for="'h_bold_'+index" class="text-[9px] text-white whitespace-nowrap">{{ __('H.Bold') }}</label>
                                         </div>
                                         <div class="flex items-center gap-1.5">
                                             <input type="checkbox" :name="'columns['+index+'][styles][content_bold]'" x-model="col.styles.content_bold" value="1" :id="'c_bold_'+index" class="rounded border-notion-border bg-white/5 text-notion-blue">
                                             <label :for="'c_bold_'+index" class="text-[9px] text-white whitespace-nowrap">{{ __('C.Bold') }}</label>
                                         </div>
                                     </div>
                                 </div>
                            </div>
                        </template>
                        <div x-show="cols.length === 0" class="text-center py-8 text-xs text-notion-text-secondary italic bg-white/[0.02] rounded-xl border border-dashed border-notion-border">
                ...
                <div class="flex gap-4 pt-6 border-t border-notion-border">
                    <button type="button" @click="open = false" class="flex-1 px-4 py-2.5 border border-notion-border rounded-notion text-sm font-medium hover:bg-white/5 transition-colors text-white">{{ __('Cancel') }}</button>
