...
<div x-data="{ 
    ...
    <div class="bg-card border border-notion-border rounded-notion overflow-hidden shadow-sm">
        <div class="p-6 border-b border-notion-border bg-white/5 flex items-center justify-between">
            <div>
                <h3 class="font-bold text-lg text-white">{{ __('Document Templates') }}</h3>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-notion-border bg-white/[0.01]">
                ...
                <tbody class="divide-y divide-notion-border">
                    @forelse($templates as $template)
                        <tr class="hover:bg-white/[0.02] transition-colors group cursor-pointer" @click="openEdit({{ json_encode($template) }})">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-sm font-medium text-white group-hover:text-notion-blue transition-colors">{{ $template->name }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <code class="text-[10px] text-notion-text-secondary bg-black/30 px-2 py-1 rounded break-all max-w-[150px] inline-block">{{ $template->google_drive_id }}</code>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-xs text-white capitalize">{{ $template->orientation }}</span>
                            </td>
                            <td class="px-6 py-4">
                                @if($template->content)
    ...
    <div x-show="open" 
        ...
        <div @click.away="open = false" class="bg-card border border-notion-border w-full max-w-4xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div class="p-4 border-b border-notion-border flex items-center justify-between">
                <h3 class="text-sm font-bold text-white" x-text="isEdit ? '{{ __('Edit Template') }}' : '{{ __('Add Document Template') }}'"></h3>
                <button @click="open = false" class="text-notion-text-secondary hover:text-white transition-colors">
            ...
            <form :action="isEdit ? '/admin/templates/' + editId : '{{ route('admin.settings.templates.store') }}'" method="POST" class="flex-1 flex flex-col min-h-0 overflow-hidden">
                ...
                <div class="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div class="space-y-2 border-t border-notion-border pt-6">
                        ...
                        <textarea name="content" x-model="template.content" rows="12" class="notion-input w-full text-[12px] font-mono leading-relaxed bg-black/40" placeholder="<p>Рахунок №@{{2001}}</p> ... @{{table:products}}"></textarea>
                        @error('content')
                ...
                <div class="p-6 border-t border-notion-border bg-white/[0.01] flex gap-4">
                    <button type="button" @click="open = false" class="flex-1 border border-notion-border text-white py-2 rounded-notion text-sm font-medium hover:bg-white/5 transition-all">
                        {{ __('Cancel') }}
                    </button>
