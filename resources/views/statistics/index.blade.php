...
<div class="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500" x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-notion-text-primary">{{ __('Statistics & Analytics') }}</h1>
            <p class="text-notion-text-secondary mt-1">{{ __('Real-time overview of your sales performance.') }}</p>
        </div>
        <form action="{{ route('statistics.index') }}" method="GET" class="flex flex-wrap items-center gap-3 bg-card p-2 rounded-xl border border-notion-border">
            <div class="flex items-center gap-2">
                <label class="text-[10px] uppercase tracking-widest text-notion-text-secondary font-bold ml-2">{{ __('From') }}</label>
                <input type="date" name="start_date" value="{{ $startDate }}" class="bg-notion-hover border-none rounded-md px-3 py-1.5 text-sm text-notion-text-primary focus:ring-1 focus:ring-notion-blue">
            </div>
            <div class="flex items-center gap-2">
                <label class="text-[10px] uppercase tracking-widest text-notion-text-secondary font-bold ml-2">{{ __('To') }}</label>
                <input type="date" name="end_date" value="{{ $endDate }}" class="bg-notion-hover border-none rounded-md px-3 py-1.5 text-sm text-notion-text-primary focus:ring-1 focus:ring-notion-blue">
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 bg-card border border-notion-border rounded-2xl p-6 shadow-xl flex flex-col">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-sm font-bold text-notion-text-secondary uppercase tracking-widest">{{ __('Manager Stats') }}</h3>
                <select x-model="selectedManagerId" class="bg-notion-hover border-none rounded-lg text-xs text-notion-text-primary py-1 px-3 focus:ring-1 focus:ring-notion-blue">
            ...
            <div class="space-y-6">
                <div class="text-center py-4 bg-notion-hover/30 rounded-2xl border border-notion-border">
                    <div class="text-[10px] text-notion-text-secondary uppercase font-bold">{{ __('Total Managed') }}</div>
                    <div class="text-3xl font-black text-notion-text-primary mt-1" x-text="selectedManager.total"></div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="h-1.5 w-full bg-notion-hover rounded-full overflow-hidden">
        ...
        <!-- BLOCK 2: Top Clients -->
        <div class="lg:col-span-2 bg-card border border-notion-border rounded-2xl shadow-xl flex flex-col overflow-hidden">
            <div class="px-6 py-5 border-b border-notion-border flex items-center justify-between">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-notion-hover/50 text-[10px] font-bold text-notion-text-secondary uppercase tracking-widest">
                    ...
                    <tbody class="divide-y divide-notion-border">
                        @foreach($clientStats as $client)
                            <tr class="hover:bg-notion-hover transition-colors border-l-2 border-transparent hover:border-notion-blue">
                                <td class="px-6 py-4">
                                    <span class="text-sm font-medium text-notion-text-primary">{{ $client['name'] }}</span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="text-sm font-bold text-notion-text-primary">{{ $client['total'] }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right text-sm font-bold text-notion-text-primary">
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-4">
            @foreach($performanceData as $p)
                <div class="bg-card border border-notion-border rounded-2xl overflow-hidden shadow-lg transition-all" 
                    ...
                    <div @click="expandedPipeline = (expandedPipeline === {{ $p['id'] }} ? null : {{ $p['id'] }})" 
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h4 class="text-lg font-bold text-notion-text-primary">{{ $p['name'] }}</h4>
                                <div class="flex items-center gap-3 mt-1">
                                    <span class="text-xs text-notion-text-secondary">{{ __('Deals total') }}: <b class="text-notion-text-primary">{{ $p['total'] }}</b></span>
                                    <span class="w-1 h-1 rounded-full bg-notion-text-mono/20"></span>
                        ...
                        <div class="flex items-center gap-4">
                            <div class="text-right hidden md:block">
                                ...
                                <div class="text-sm font-bold text-notion-text-primary">{{ $p['total'] > 0 ? round(($p['won'] / $p['total']) * 100, 1) : 0 }}%</div>
                            </div>
                    ...
                    <div x-show="expandedPipeline === {{ $p['id'] }}" 
                         x-collapse
                         class="bg-notion-hover/10 border-t border-notion-border p-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                            <div class="p-4 bg-notion-hover/30 rounded-xl border border-notion-border">
                                <div class="text-[10px] text-notion-text-secondary font-bold uppercase">{{ __('New in Period') }}</div>
                                <div class="text-2xl font-bold text-notion-text-primary mt-1">{{ $p['total'] }}</div>
                            </div>
                        ...
                        <div class="space-y-3">
                            @foreach($p['stages'] as $stage)
                                <div class="flex items-center gap-4 group">
                                    ...
                                    <div class="flex-1 h-3 bg-notion-hover rounded-full overflow-hidden relative">
                                    ...
                                    <div class="w-12 text-right text-xs font-bold text-notion-text-primary">
                                        {{ $stage['count'] }}
                                    </div>
                                </div>
                            @endforeach
